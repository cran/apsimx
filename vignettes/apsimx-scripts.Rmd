...
Notice that for each new build of APSIM-X the path will change. After 'APSIM' is the year and month, and version separated by a period. In this case '2024.2.7382.0'. In the folder 'bin' (for 'binary') there are many libraries ('.dll files') and some executables. The 'Models.exe' is the one that can run .apsimx files.
...
[Editing PATH in Windows Example](https://www.telerik.com/teststudio/documentation/features/test-runners/add-path-environment-variables)
